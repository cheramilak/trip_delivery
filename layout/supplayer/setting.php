...
if(isset($_POST['submit'])){
  ...
  if(empty($name) || empty($address) || empty($phone)){
   ...
   $admin = "SELECT *FROM  supplayer WHERE user_id = '$user_id'";
   $result = mysqli_query($connection,$admin);
   $count = mysqli_num_rows($result);
   if(mysqli_num_rows($result)>0){
    $sql = "UPDATE  `supplayer` SET `phone` = $phone WHERE `user_id` = $user_id"; 
    mysqli_query($connection,$sql);
    $sql = "UPDATE  `supplayer` SET `address` = '$address' WHERE `user_id` = $user_id"; 
    mysqli_query($connection,$sql);
   }
   

   else{
    $sql = "INSERT INTO supplayer(user_id,phone,address) VALUES($user_id,$phone,'$address')";
    mysqli_query($connection,$sql);
   }
  }
  ...
  if(isset($_POST['upload'])){
    ...
    move_uploaded_file($newImageName,'/image');
    $sql = "UPDATE  `supplayer` SET `image` = '$newImageName' WHERE `id` = '$user_id'"; 
    mysqli_query($connection,$sql);
...
$supplayer = "SELECT *FROM supplayer WHERE user_id = $user_id";

$result = mysqli_query($connection,$supplayer);

  $supplayer = mysqli_fetch_array($result,MYSQLI_ASSOC);
    ...
    <div class="container">
      ...
      <div class="settings">
        <div class="setting__left--side">
          ...
          <a class="settings_account-inner" href="setting.php?password">
            <div class="account_img">
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div class="setting__right--side-inner">
            ...
            <div class="setting__right--side-input">
              ...
              <div class="input-box">
                ...
                <input type="number" name="number" id="number" value="<?php echo$supplayer['phone'] ?>" required placeholder="(+251) 555-0113">
              </div>
              <div class="input-box">
                <label for="number"><Address>Addresse</Address></label>
                <input type="text" name="address" value="<?php echo$supplayer['address'] ?>" id="number" required placeholder="Block and dorm number">
              </div>
            </div>
        ...
        <div class="setting__right--side-input" style="margin-top: 100px;">
              <div class="input-box" style="display: inline;">
                <label for="oldPassword">Old Password</label>
                <input type="password" name="oldPassword"   id="email"  required placeholder="Old password">
              </div>
              <br>
              <div class="input-box">
                <label for="newPassword">New Password</label>
                <input type="password" name="newPassword"   id="email"  required placeholder="New password">
              </div>
              <div class="input-box">
                <label for="confirmPassword">confirm Password</label>
                <input type="password" name="confirmPassword"   id="email"  required placeholder="Confirm password">
              </div>
              <div><input class="SaveBtn" name="changePassword" value="Save changes" type="submit"></div>
